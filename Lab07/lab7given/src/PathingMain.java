...
public class PathingMain extends PApplet
{
	...
	*/
   private boolean findGoal(Point pos, GridValues[][] grid, List<Point> path)
   {

		//if i am valid grid cell
		if (withinBounds(pos, grid)  &&
         grid[pos.y][pos.x] != GridValues.OBSTACLE &&
         grid[pos.y][pos.x] != GridValues.SEARCHED)
      {
			   //check if i am  the goal
            ...
            if( findGoal(right, grid, path) || findGoal(left, grid, path) || findGoal(down, grid, path) || findGoal(up, grid, path) )
            {
               path.add(pos);
               return true;
            }
